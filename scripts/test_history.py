...
cmab, opt_rewards, opt_actions, num_actions, context_dim, _ = load_ag_news(100, 20, 42)
arms, pb = build_ag_news_arms_and_pb()

res = run_our_method_pipeline(
    cmab=cmab, opt_rewards=opt_rewards, opt_actions=opt_actions,
    n_rounds=100, num_actions=num_actions, context_dim=context_dim,
...
for i, rec in enumerate(res["history"][:10]):
    print(f"Step {i}: min_source={rec.min_source}")
