...
class NeuralBanditModel(BayesianNeuralNetwork):
    ...
    def __init__(self, hparams: Dict[str, Any], name: str = "neural_bandit"):
        ...
        self.verbose = hparams.get("verbose", True) #是否打印模型信息
        self.times_trained = 0 #记录模型训练次数
        
        # Extract hyperparameters
        self.context_dim = hparams["context_dim"] #上下文维度
        self.num_actions = hparams["num_actions"] #动作数量
    ...
    def build_model(self):
        ...
        layers = [] #存储网络层的列表
        input_dim = self.context_dim #输入维度等于上下文维度
        
        # Build hidden layers
        for layer_size in self.layer_sizes: #遍历每个隐藏层大小
            if layer_size > 0:
                layers.append(nn.Linear(input_dim, layer_size)) #添加线性层，维度为input_dim到layer_size
                
                if self.layer_norm:
                    layers.append(nn.LayerNorm(layer_size)) #添加层归一化层
                ...
                if self.use_dropout:
                    layers.append(nn.Dropout(self.dropout_rate)) #添加Dropout层
                    
                input_dim = layer_size #更新输入维度为当前层大小
        
        # Output layer
        layers.append(nn.Linear(input_dim, self.num_actions)) #添加输出层，维度为最后隐藏层到动作数量
        
        self.network = nn.Sequential(*layers) #使用Sequential封装网络层
        
        # Initialize weights
        self._initialize_weights()
        
        # Setup optimizer
        self.optimizer = optim.Adam(self.parameters(), lr=self.learning_rate) #使用Adam优化器，学习率为self.learning_rate
