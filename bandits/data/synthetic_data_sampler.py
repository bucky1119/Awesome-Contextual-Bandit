...
def sample_linear_data(num_contexts, dim_context, num_actions, sigma=0.0):
  ...
  betas = np.random.uniform(-1, 1, (dim_context, num_actions)) # 随机均匀采样
  betas /= np.linalg.norm(betas, axis=0) # 对每个动作的参数向量进行归一化
  contexts = np.random.normal(size=[num_contexts, dim_context]) # 采样上下文，高斯分布,维度（num_contexts, dim_context）
  rewards = np.dot(contexts, betas) # 计算奖励，线性函数，上下文矩阵点乘参数矩阵，维度（num_contexts, num_actions）
  opt_actions = np.argmax(rewards, axis=1) # 最优动作，选择奖励最大的动作
  rewards += np.random.normal(scale=sigma, size=rewards.shape) # 添加高斯噪声，模拟真实环境，通过sigma控制噪声大小
  opt_rewards = np.array([rewards[i, act] for i, act in enumerate(opt_actions)]) # 最优奖励
  return np.hstack((contexts, rewards)), betas, (opt_rewards, opt_actions) # 横向拼接上下文和奖励，返回数据集（上下文，奖励）、参数、最优奖励和动作
